...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof dcusumkbn2, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 1', function test( t ) {
	t.strictEqual( dcusumkbn2.length, 1, 'has expected arity' );
	t.end();
});
...
tape( 'the function computes the cumulative sum of a one-dimensional ndarray', function test( t ) {
	...
	x = vector( xbuf, 2, 1, 0 );
	y = zerosLike( x );
	initial = scalar2ndarrayLike( x, NaN );
	v = dcusumkbn2( [ x, y, initial ] );
...
tape( 'if provided an empty ndarray, the function returns the output array unchanged', function test( t ) {
	...
	x = vector( xbuf, 0, 1, 0 );
	y = zerosLike( x );
	initial = scalar2ndarrayLike( x, 100.0 );
